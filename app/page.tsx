...
function PortfolioLanding({state,dispatch}:{state:GameState;dispatch:(a:Action)=>void}){
  ...
  return <div className="portfolio-home">
    ...
    <header className="site-header">
      <a className="site-brand" href="/" onClick={scrollToTop} aria-label="Ignacio Schwindt — home"><span>IS</span><b>IGNACIO SCHWINDT</b></a>
      ...
      <div className="site-actions">
        <div className="site-lang-switch" role="group" aria-label={text.language}>
          <Globe2 size={13} aria-hidden="true" className="lang-globe"/>
          <button type="button" className={!es?"active":""} onClick={()=>dispatch({type:"LANG",language:"en"})} aria-pressed={!es} title="Switch to English">EN</button>
          <span className="lang-sep" aria-hidden="true">|</span>
          <button type="button" className={es?"active":""} onClick={()=>dispatch({type:"LANG",language:"es"})} aria-pressed={es} title="Cambiar a Español">ES</button>
        </div>
        <Link className="nav-game" href="/game/"><Play/>{es?"Jugar":"Play"}</Link>
      </div>
    </header>
    ...
    <footer className="site-footer"><a className="site-brand" href="/" onClick={scrollToTop}><span>IS</span><b>IGNACIO SCHWINDT</b></a><p>{text.built}</p><span>© {new Date().getFullYear()}</span></footer>
  </div>
}
